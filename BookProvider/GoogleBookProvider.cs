...
namespace BookatHomeProvider
{
    public class GoogleBookProvider : BooksProvider
    {
        ...
        public override List<PocoBook> FetchBooksInfo(PocoBook book, IBookParserTrace trace)
        {
            Rootobject rootresponse = ExecuteGoogleRequest(book, trace);

            return GetBookFromRest(rootresponse, book.File);
        }


        private  Rootobject ExecuteGoogleRequest(PocoBook book, IBookParserTrace trace)
        {
            trace.TraceInfo("FetchBooksInfo start for book {0}", book.Title);
